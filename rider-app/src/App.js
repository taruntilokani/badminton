...
function App() {
  ...
  const [requests, setRequests] = useState([
    { id: "REQ123456", status: "Waiting for pickup", riderTime: 0, otp: "111111" },
    { id: "REQ654321", status: "Waiting for pickup", riderTime: 0, otp: "222222" }
  ]);
  const [activeOtp, setActiveOtp] = useState('');
  // Sample login
  const TEST_USER = 'rider@example.com';
  const TEST_PASS = 'rider123';
  ...
  const handleLogin = e => {
    ...
    if (user === TEST_USER && pass === TEST_PASS) {
      ...
      setError('Invalid credentials. Use rider@example.com / rider123');
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: mainBg, fontFamily: font, padding: 0, margin: 0 }}>
      <div style={{ maxWidth: 480, margin: '48px auto', background: cardBg, borderRadius: 16, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: `1px solid ${border}`, padding: 32 }}>
        <h2 style={{ color: accent, fontWeight: 700, marginBottom: 24, textAlign: 'center', letterSpacing: 1 }}>Rider Dashboard</h2>
        {!loggedIn ? (
          showSignup ? (
            <RiderSignup onSuccess={() => setShowSignup(false)} />
          ) : (
            <>
          ...
          <div>
            <h3 style={{ color: accent, fontWeight: 600, marginBottom: 16 }}>Pickup Requests</h3>
            {requests.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {requests.map((r, idx) => (
                  <div key={r.id} style={{ background: mainBg, borderRadius: 12, border: `1px solid ${border}`, padding: 20, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                    ...
                    {r.status === "Waiting for pickup" && (
                      <form onSubmit={e => {
                        e.preventDefault();
                        if (activeOtp === r.otp) {
                          const updated = [...requests];
                          updated[idx].status = "Picked up";
                          updated[idx].riderTime = 10; // Simulate time
                          setRequests(updated);
                          setActiveOtp('');
                        }
                        ...
                        <button type="submit" style={{ background: accent, color: '#fff', border: 'none', borderRadius: 6, padding: '8px 16px', fontWeight: 600, cursor: 'pointer' }}>Verify Pickup</button>
                      </form>
                    )}
                    {r.status === "Picked up" && <div style={{ color: '#388e3c', fontWeight: 500, marginTop: 8 }}>Racket picked up. Deliver to vendor.</div>}
                  </div>
                ))}
              </div>
            )}
